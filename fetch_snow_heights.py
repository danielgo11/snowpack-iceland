...
SENSORS = [
    (210, "Steinidjugil", 66.076302, -23.158573, 420),
    (75, "Ljósavatnsskarð", 65.718079, -17.678465, 562),
    (78, "Flateyri Midhryggsgil", 66.061745, -23.485704, 610),
...
def write_smet_file(df: pd.DataFrame, sensor_id: int, sensor_name: str, 
    ...
    output_file = OUTPUT_DIR / ("%d_%s_hs.txt" % (sensor_id, sensor_name.replace(" ", "_").lower()))
...
def write_report(sensor_id: int, sensor_name: str, report: Dict, output_file: str):
    ...
    report_file = OUTPUT_DIR / ("%d_%s_report.txt" % (sensor_id, sensor_name.replace(" ", "_").lower()))
